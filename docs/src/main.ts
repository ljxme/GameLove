...
import { HostsDataSource } from './services/hostsService';
import { PerformanceMonitor } from './utils/performance';
import { ConnectivityTester } from './services/connectivityService';
...
class HostsConnectivityChecker {
  ...
  private connectivityTester: IConnectivityTester;
  ...
  private async loadHostsData(): Promise<void> {
    try {
      // 使用模块化数据源，包含主/备/本地回退策略
      const dataSource = new HostsDataSource();
      const hostsData = await dataSource.load();
      this.parseHostsData(hostsData);
    } catch (error) {
      console.error('加载 hosts 数据失败:', error);
   ...
   * 解析hosts数据
   */
  private parseHostsData(hostsData: HostsJson): void {
  ...
  private createPlatformElement(platform: PlatformInfo): HTMLElement {
    ...
    platformDiv.className =
      'bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02]';

    const successCount = platform.domains.filter(
      (d) => d.status === ConnectivityStatus.SUCCESS
    ).length;
    const totalCount = platform.domains.length;
    const testingCount = platform.domains.filter(
      (d) => d.status === ConnectivityStatus.TESTING
    ).length;
    const errorCount = platform.domains.filter((d) => d.status === ConnectivityStatus.ERROR).length;
    const warningCount = platform.domains.filter(
      (d) => d.status === ConnectivityStatus.WARNING
    ).length;
  ...
  private createDomainElement(domainInfo: DomainInfo): HTMLElement {
    ...
    switch (domainInfo.status) {
      case ConnectivityStatus.SUCCESS:
        ...
        statusBadge =
          '<span class="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full font-medium">正常</span>';
        break;
      case ConnectivityStatus.WARNING:
        statusClass = 'bg-yellow-50 border-l-4 border-yellow-400';
        statusBadge =
          '<span class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full font-medium">警告</span>';
        break;
      case ConnectivityStatus.ERROR:
        statusClass = 'bg-red-50 border-l-4 border-red-400';
        statusBadge =
          '<span class="px-2 py-1 bg-red-100 text-red-800 text-xs rounded-full font-medium">异常</span>';
        break;
      case ConnectivityStatus.TESTING:
        statusClass = 'bg-blue-50 border-l-4 border-blue-400 animate-pulse';
        statusBadge =
          '<span class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full font-medium animate-pulse">检测中</span>';
        break;
      default:
        statusClass = 'bg-gray-50 border-l-4 border-gray-300';
        statusBadge =
          '<span class="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full font-medium">未知</span>';
    }
  ...
  private addTestCompleteVisualEffects(): void {
    // 为成功的域名添加庆祝动画
    const successDomains = this.getAllDomains().filter(
      (d) => d.status === ConnectivityStatus.SUCCESS
    );
  ...
  private enableResultExport(): void {
    ...
    if (!exportBtn) {
      ...
      exportBtn.className =
        'px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-200 shadow-lg transform hover:scale-105';
  ...
  private async processDomainsBatch(domains: DomainInfo[]): Promise<void> {
    ...
    for (let i = 0; i < domains.length; i += batchSize) {
      ...
      // 并行处理当前批次
      await Promise.allSettled(batch.map((domain) => this.testDomainWithRetry(domain)));
   ...
   * 显示批次完成状态
   */
  private showBatchCompleteStatus(
    currentBatch: number,
    totalBatches: number,
    batch: DomainInfo[]
  ): void {
  ...
  private createRippleEffect(element: HTMLElement): void {
    ...
    ripple.className =
      'absolute inset-0 bg-blue-400 opacity-20 rounded-lg animate-ping pointer-events-none';
  ...
  private async testDomainConnectivity(domainInfo: DomainInfo): Promise<void> {
    ...
    try {
      ...
      // 尝试多种检测方法
      await this.connectivityTester.test(domainInfo.domain, combinedSignal);
  ...
  private updateProgressBar(): void {
    ...
    if (progressBarElement && progressTextElement) {
      ...
      const progressPercentage =
        this.statistics.total > 0 ? Math.round((testedCount / this.statistics.total) * 100) : 0;
  ...
  private updatePlatformStats(platformName?: string): void {
    ...
    platforms.forEach((name) => {
      ...
      if (platform) {
        ...
        const errorCount = platform.domains.filter(
          (d) => d.status === ConnectivityStatus.ERROR
        ).length;
        const testingCount = platform.domains.filter(
          (d) => d.status === ConnectivityStatus.TESTING
        ).length;
          ...
          if (indicatorElement) {
            if (testingCount > 0) {
              indicatorElement.className =
                'platform-indicator w-3 h-3 rounded-full bg-blue-500 animate-pulse';
  ...
  private updateTestAllButton(isTesting: boolean): void {
    ...
    if (button) {
      if (isTesting) {
        ...
        button.className =
          'px-6 py-3 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-200 font-medium shadow-lg transform hover:scale-105';
        button.disabled = false;
      } else {
        const hasResults =
          this.statistics.success + this.statistics.warning + this.statistics.error > 0;
        ...
        button.className =
          'px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-200 font-medium shadow-lg transform hover:scale-105';
        button.disabled = false;
      }
    }
  ...
  private getFastestDomain(): { domain: string; responseTime: number } | null {
    ...
    const successDomains = allDomains.filter(
      (d) => d.status === ConnectivityStatus.SUCCESS && d.responseTime !== undefined
    );
  ...
  private createResultModal(data: {
    ...
    modal.className =
      'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fadeInUp';

    const content = document.createElement('div');
    content.className =
      'bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-2xl transform transition-all duration-300';
